 ...
 */
    public apiResponseHandler(lambdaResponse, gatewayEvent) {
        ...
        if (lambdaResponse) {
            ...
            apiResponse.setBody(this.createFinalResponse(['SIPMS001'], lambdaResponse));
 ...
 */
    ...
    private createFinalResponse(messageCodes, lambdaResponse) {
        ...
        for (let i = 0; i < messageCodes.length; i++) {
            ...
            messages.setDetailedDescription(msg[messageCodes[i]]["detailedDescription"]);
            status.setCode(msg[messageCodes[i]].httpCode);
            status.setErrorCode(msg[messageCode]["code"]);
            if ("SIPMS001" === messageCodes[i]) {
                status.setErrorCode("");
            }
